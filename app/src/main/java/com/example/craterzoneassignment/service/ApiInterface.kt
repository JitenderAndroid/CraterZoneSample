...
import com.example.craterzoneassignment.data.ApiResponse
...
interface ApiInterface {
    ...
    fun getImages(@Query("method") method: String = METHOD,
                          @Query("api_key") apiKey: String  = API_KEY,
                          @Query("format") format: String = "json",
                          @Query("text") text: String,
                          @Query("per_page") perPage: String = "15",
                          @Query("page") page: Int = 1,
                          @Query("nojsoncallback") nojsoncallback: String = "1"): LiveData<ApiResponse<ImageResponse>>
}